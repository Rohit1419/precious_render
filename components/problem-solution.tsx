...
const DEFAULT_PAIN_POINTS = [
  {
    ...
    riskText: "Risk: Unsold inventory becomes dead capital",
  },
  {
    icon: "package",
    ...
    riskText: "Risk: Theft, damage, or degradation of precious materials",
  },
  {
    icon: "clock",
    title: "Slow Market Response",
    description:
      "Spotted a trending design on Instagram? By the time you manufacture 500 pieces, the trend has moved on.",
    riskText: "Risk: Overproduction of unpopular designs",
  },
];
...
export default function ProblemSolution({ data }: ProblemSolutionProps) {
  const openingHeadlinePrimary =
    data?.openingHeadlinePrimary ?? "Why Traditional Jewelry Inventory";
  const openingHeadlineAccent =
    data?.openingHeadlineAccent ?? "Is Holding You Back";
  const openingSubheadline =
    data?.openingSubheadline ??
    "Physical inventory comes with hidden costs that eat into your profits and slow down your growth";

  const painPoints = data?.painPoints?.length
    ? data.painPoints
    : DEFAULT_PAIN_POINTS;
  const benefits = data?.benefits?.length ? data.benefits : DEFAULT_BENEFITS;

  const solutionBadge =
    data?.solutionBadge ?? "The Virtual Inventory Revolution";
  const solutionHeading =
    data?.solutionHeading ?? "From CAD to Catalog - Without Stock";
  ...
  const solutionCtaHref =
    data?.solutionCtaHref ??
    "https://wa.me/+918087881239?text=Hello%20I%20want%20to%20know%20more%20about%20your%20services";

  const compareBeforeImage =
    data?.compareBeforeImage ?? "/images/cad-render/Banner_Clay.webp";
  const compareAfterImage =
    data?.compareAfterImage ?? "/images/cad-render/Banner_Render.webp";
  ...
  const caseStudyHeading =
    data?.caseStudyHeading ?? "Mumbai Jeweler Cuts Inventory by 80%";
  ...
  const caseStudyStats = data?.caseStudyStats?.length
    ? data.caseStudyStats
    : DEFAULT_CASE_STUDY_STATS;
  ...
  const traditionalImage =
    data?.traditionalImageUrl ?? "/images/image%20(4).png";
  const traditionalSteps = data?.traditionalSteps?.length
    ? data.traditionalSteps
    : DEFAULT_TRADITIONAL_STEPS;
  const traditionalResult =
    data?.traditionalResult ?? "Result: 30-40% unsold inventory";

  const virtualHeading = data?.virtualHeading ?? "Virtual Inventory Model";
  const virtualBadge = data?.virtualBadge ?? "RECOMMENDED";
  const virtualImage =
    data?.virtualImageUrl ?? "/images/e61edaea-7a28-4ae4-9eac-33a58c09fcf9.png";
  const virtualSteps = data?.virtualSteps?.length
    ? data.virtualSteps
    : DEFAULT_VIRTUAL_STEPS;
  const virtualResult =
    data?.virtualResult ?? "Result: 0% unsold stock, 100% profit";

  const closingHeading =
    data?.closingHeading ?? "Ready to Build Your Virtual Jewelry Showroom?";
    ...
    <section className="py-16 md:py-24 relative overflow-hidden bg-white dark:bg-neutral-950">
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-24">
          {painPoints.map((point, i) => {
            ...
            return (
              <MagicCard
                key={i}
                className="p-8 rounded-2xl border-t-4 border-red-500 bg-red-50/50 dark:bg-red-900/10 h-full"
              >
        ...
        <div className="mb-24 relative">
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-3xl border border-emerald-100 dark:border-emerald-900/30 p-8 md:p-12 shadow-xl">
            <div className="flex flex-col items-center text-center">
              ...
              <div className="w-full max-w-5xl mx-auto h-[200px] sm:h-[300px] md:h-[400px] lg:h-[500px] rounded-2xl overflow-hidden shadow-2xl mb-10">
                <CompareSlider
                  beforeImage={compareBeforeImage}
                  afterImage={compareAfterImage}
                  beforeLabel={compareBeforeLabel}
              ...
              <div className="text-center max-w-3xl mx-auto">
                <CTAButton href={closingCtaHref}>{closingCtaLabel}</CTAButton>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-24">
          {benefits.map((benefit, i) => {
            ...
            return (
              <div
                key={i}
                className="p-6 rounded-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 hover:border-emerald-500 dark:hover:border-emerald-500 transition-colors group"
              >
                <div className="flex items-center gap-3 mb-4 flex-nowrap">
                  ...
                  <h3 className="text-xl font-bold text-neutral-900 dark:text-white min-w-0">
                    {benefit.title}
                  </h3>
                </div>
            ...
            <div className="bg-neutral-50 dark:bg-neutral-900/50 p-8 md:p-12 rounded-xl">
              <div className="flex flex-col md:flex-row gap-10 items-center">
                <div className="flex-1">
                  <div className="text-sm font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider mb-3">
                    {caseStudyLabel}
                  </div>
                ...
                <div className="bg-white dark:bg-neutral-800 p-6 rounded-xl shadow-lg border border-neutral-100 dark:border-neutral-700 w-full md:w-auto min-w-[300px]">
                  <ul className="space-y-4">
                    {caseStudyStats.map((stat, i) => (
                      <li
                        key={i}
                        className="flex items-center text-neutral-700 dark:text-neutral-300 font-medium"
                      >
        ...
        <div className="mb-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
            ...
            <div className="bg-red-50/30 dark:bg-red-900/10 rounded-2xl p-8 border border-red-100 dark:border-red-900/20">
              ...
              <div className="aspect-video bg-neutral-200 dark:bg-neutral-800 rounded-lg mb-8 overflow-hidden relative">
                <Image
                  ...
                  alt="Traditional jewelry showcase"
                  fill
              ...
              <ul className="space-y-4">
                {traditionalSteps.map((step, i) => (
                  ...
                  >
                    <span className="w-6 h-6 rounded-full bg-red-100 dark:bg-red-900/30 text-red-600 text-sm font-bold flex items-center justify-center mr-3">
                      {i + 1}
                    </span>
                    {step}
                  </li>
            ...
            <div className="bg-emerald-50/30 dark:bg-emerald-900/10 rounded-2xl p-8 border border-emerald-100 dark:border-emerald-900/20 relative overflow-hidden">
              ...
              <h3 className="text-2xl font-bold mb-6 text-emerald-700 dark:text-emerald-400 text-center">
                {virtualHeading}
              </h3>
              <div className="aspect-video bg-neutral-200 dark:bg-neutral-800 rounded-lg mb-8 overflow-hidden relative">
                <Image
                  src={virtualImage}
                  alt="Digital virtual jewelry catalog"
                  fill
                  sizes="(max-width: 768px) 100vw, 50vw"
                  className="object-cover"
                />
              ...
              <ul className="space-y-4">
                {virtualSteps.map((step, i) => (
                  <li
                    ...
                    className="flex items-center text-neutral-700 dark:text-neutral-300"
                  >
                    <span className="w-6 h-6 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 text-sm font-bold flex items-center justify-center mr-3">
                      {i + 1}
                    </span>
                    {step}
                  </li>
        ...
        <div className="text-center max-w-3xl mx-auto">
          ...
          <a
            href={closingCtaHref}
            className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-emerald-500 to-cyan-500 text-white rounded-xl font-bold text-lg hover:shadow-lg hover:scale-105 transition-all"
          >
            {closingCtaLabel}
